...
import 'package:school_companion_app/components/samplepapers/samplepapers.dart';
...
class SamplePaperDataForm extends StatefulWidget {
  _SamplePaperDataFormState createState() => _SamplePaperDataFormState();
}

class _SamplePaperDataFormState extends State<SamplePaperDataForm> {
  //Academic Years
...
void _submit(BuildContext context, String subject) async {
  try {
    Map<String, dynamic> x = await datastore.getSamplePapers(
        subject, datastore.getUserInfo()['grade']);
        if(x['response'].length > 0){
          Navigator.push(
        context,
        MaterialPageRoute(
            builder: (BuildContext context) => SamplePaperView(x)));
        }else{
          showDialog(
        context: context,
        builder: (_) => new AlertDialog(
              title: new Text("Sample Paper Unavailable"),
              content: Container(
                  height: 100.0,
                  child: Center(
                    child: Text(
                        "The Institution has not uploaded any ${datastore.getSubject(subject)} Sample Question Papers. We are sorry for the inconvenience."),
                  )),
            ));
        }
  } catch (e) {
